...
template<typename T>
auto simpson_similarity(const T x1, const T x2) -> f64 {
    f64 prod_num = 0;
    for(auto itr=x1.begin(); itr!=x1.end(); ++itr) {
        prod_num += x2.count(*itr);
    }

    return prod_num / min(x1.size(), x2.size());
}
...
template f64 simpson_similarity(const set<i32>, const set<i32>);
template f64 simpson_similarity(const set<i64>, const set<i64>);
template f64 simpson_similarity(const set<f64>, const set<f64>);
} // namespace similarity
} // namespace cpplearn
